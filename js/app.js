...
async function loadConfig(){
  ...
  imgBtn.addEventListener('click', (e) => { e.preventDefault(); openImageSlider(); });
  heroLinks.appendChild(imgBtn);
  [['LinkedIn', cfg.links.linkedin], ['GitHub', cfg.links.github]].forEach(([label, url]) => {
  ...
  cfg.projects.forEach(p => {
    ...
    el.innerHTML = `
      ...
      <div class="card-footer">
        ${p.links.demo ? `<a href="${p.links.demo}" target="_blank" rel="noopener noreferrer" class="demo-link">Demo</a>` : ''}
        <a href="${p.links.repo}" target="_blank" rel="noopener noreferrer" class="github-icon" aria-label="View ${p.title} source code on GitHub">
  ...
  Object.entries(cfg.links).forEach(([k,v]) => {
    ...
    contact.innerHTML += `<a href="${v}" target="_blank" rel="noopener noreferrer">${nice}</a> · `;
  });
